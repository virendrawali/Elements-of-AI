...
class Score:
    ...
    def print_helper(description, list, sentence):
        print (("%41s" % description) + " " + " ".join([(("%-" + str(max(4,len(sentence[i]))) + "s") % list[i]) for i in  range(0,len(list)) ] ) )
    ...
    def print_results(sentence, outputs, posteriors, models):
        ...
        for algo in sorted(outputs.keys()):
            Score.print_helper(algo + "  "+" ".join(["%9.4f" % posteriors[algo][model] for model in models]), outputs[algo], sentence)
